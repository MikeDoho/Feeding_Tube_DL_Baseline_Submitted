...
from lib.Models import resnet_fork_spatial as resnet_fork
...
def generate_model(opt):
    ...
    if opt.model == 'resnet':
        ...
        if opt.in_modality == 3 and opt.is_classi:
            print('input CBCT, CT, Dose as 3 channels for classification')

            if opt.model_depth == 10:
                model = resnet_fork.resnet10(
    ...
    if opt.phase != 'test' and opt.pretrain_path and opt.is_transfer:
        ...
        print('new layer names: ', opt.new_layer_names)
        assert isinstance(opt.new_layer_names, list), f"needs to be list: {opt.new_layer_names}"
